...
# Create and rotate a triangle about the fixed point
...
def draw():                                            	# ondraw is called all the time
    ...
    '''
    To rotate the triangle about the fix point.
    
    Let the fixed point be the left vertex of triangle.
    glRotatef() function rotate around the origin
    so, we can't directly use goRotatef() function
    
    Steps to follow for rotation the triangle about fix point
    
    1) we will translate about the fix point (left vertex in this case)
    2) Then we will use glRotatef() to rotate about the fix point
    3) Then agin we translate back to the normal.
